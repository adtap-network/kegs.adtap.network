...
class QuotesService extends CommonService {
    ...
    constructor(context: CommonContext) {
        ...
        if(this.context.datastore == 'my') { this.adapter = new MysqlAdapter(this.context.datasources.my.adtap, new QuotesModel()); }
        else { this.adapter = new PgAdapter(this.context.datasources.pg.adtap, new QuotesModel()); }
    }
