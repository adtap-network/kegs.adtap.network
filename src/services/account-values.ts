...
class AccountValuesService extends CommonService {
    ...
    constructor(context: CommonContext) {
        ...
        if(this.context.datastore == 'my') { this.adapter = new MysqlAdapter(this.context.datasources.my.adtap, new AccountValuesModel()); }
        else { this.adapter = new PgAdapter(this.context.datasources.pg.adtap, new AccountValuesModel()); }
    }
